// Re-verify capture for the post-fix run. Identical to capture.mjs but
// writes *-after.png so before/after sit side-by-side in assets/.
...
async function captureAt(browser, ctxOpts, fileSuffix) {
  ...
  await page.screenshot({
    path: join(ASSETS, `landing-${fileSuffix}-after.png`),
    fullPage: true,
  })
  ...
  for (const s of sections) {
    try {
      ...
      await el.screenshot({
        path: join(ASSETS, `${s.label}-${fileSuffix}-after.png`),
        timeout: 5000,
      })
    } catch (e) {
...
async function main() {
  ...
  console.log("after captures complete: assets/")
}
