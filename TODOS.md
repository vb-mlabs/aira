...
- [ ] Ship web deploy for feat/business-logo — the OTA'd mobile bundle links to airabynisarga.com/legal which currently 404s. Priority: high (user-visible broken links). — *expo 2026-08-04-1715, 2026-08-04*
- [ ] Wire Sentry via @sentry/react-native Expo config plugin — shipping OTAs without crash reporting is flying blind. Requires a native rebuild (config plugin). — *expo 2026-08-04-1715, 2026-08-04*
- [ ] [HIGH — next EAS native build] Restrict Android intent filter to /verify* and /reset-password* only. Current apps/mobile/app.config.ts has no pathPrefix, so autoVerify + assetlinks handle_all_urls makes the app intercept every airabynisarga.com URL — bounces Legal/Privacy/About links back into expo-router with 'unmatched routes'. Add data entries with pathPrefix per iOS AASA. Then delete apps/mobile/lib/external-web-url.ts and drop the www rewrite in privacy-security/terms/about/index.tsx. — *hotfix mobile-android-legal-links, 2026-08-04*
